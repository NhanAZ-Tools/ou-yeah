(() => {
  ...
  async function initializeQuizTrainer() {
    ...
    if (!latestState && !looksLikePracticeQuiz()) return;
  ...
  function mountPanel(quizId, state) {
    ...
    action?.addEventListener("click", () => {
      toggleQuizTrainer(quizId).catch(handleAutomationError);
    });
  ...
  function createPublicData(state, assetPaths, exportedAt) {
    return {
      ...
      collection: {
        ...
        answerStrategy: `Chọn phương án đầu tiên cho mỗi nhóm đáp án; câu không hỗ trợ được để trống và đọc đáp án từ trang xem lại. Dừng khi ${NO_NEW_QUESTION_STREAK_LIMIT} lượt liên tiếp không có câu mới, tối đa ${state.maxAttempts} lượt.`
      },
  ...
  function renderBundleReadme(data) {
    return `# Gói bộ đề OU Yeah!\n\n`
      + `Gói này chứa bộ câu hỏi được tổng hợp từ ${data.collection.completedAttempts} lượt làm quiz \`${data.quiz.title}\`.\n\n`
