...
const HLS_SEGMENT_RETRY_DELAYS = [500, 1200, 2500]
...
chrome.runtime.onMessage.addListener((message) => {
  if (message?.type === "ou-yeah-download-hls") {
    downloadHls(message).catch((error) => {
      ...
      else sendError(message.jobId, readableError(error))
    })
  }
  ...
  if (message?.type === "ou-yeah-cancel-download-hls" && message.jobId) {
...
async function downloadHls({ jobId, url, filename }) {
  ...
  try {
    ...
    const playlist = await loadPlaylist(url, 0, controller.signal)
    ...
    async function worker() {
      while (nextIndex < playlist.segments.length) {
        ...
        const buffer = await fetchSegment(segment, controller.signal)
...
async function loadPlaylist(url, depth, signal) {
  ...
  if (variants.length) {
    ...
    return loadPlaylist(variants[0].url, depth + 1, signal)
  }
...
async function fetchSegment(segment, signal) {
  let lastError = null
  for (let attempt = 0; attempt <= HLS_SEGMENT_RETRY_DELAYS.length; attempt += 1) {
    ...
    try {
      ...
      lastError = error
      if (attempt >= HLS_SEGMENT_RETRY_DELAYS.length) break
      await delay(HLS_SEGMENT_RETRY_DELAYS[attempt])
    }
  }
  throw lastError || new Error("Không tải được segment video.")
}
...
function isCancellationError(error) {
  return error?.name === "AbortError" || error?.code === "OU_YEAH_DOWNLOAD_CANCELED"
}
