(() => {
  ...
  if (IS_TOP_FRAME) {
    /** @type {any} */ (window).OUYeahForumExportApi = Object.freeze({
      async collectBundle(sourceUrl) {
        const exported = await exportWholeForum(sourceUrl)
        return {
          exported: publicExportData(exported),
          files: buildExportBundle(exported).files
        }
      }
    })
  }
  ...
  async function attachImageAssets(exported) {
    ...
    const worker = async () => {
      while (cursor < entries.length) {
        ...
        try {
          const fetchUrl = sameOriginUrl(entry.sourceUrl)
          if (!fetchUrl) throw new Error("Ảnh ngoài miền không được tải tự động.")
          const response = await fetch(fetchUrl, { credentials: "include", redirect: "follow" })
  ...
  async function attachAttachmentAssets(exported) {
    ...
    const worker = async () => {
      while (cursor < entries.length) {
        ...
        try {
          const fetchUrl = sameOriginUrl(entry.sourceUrl)
          if (!fetchUrl) throw new Error("Tệp đính kèm ngoài miền không được tải tự động.")
          const response = await fetch(fetchUrl, { credentials: "include", redirect: "follow" })
          if (!response.ok) throw new Error(`HTTP ${response.status}`)
  ...
  async function fetchHtmlDocument(url) {
    ...
    if (!fetchUrl) throw new Error("Chỉ tải trang diễn đàn từ miền ELOLMS hiện tại.")
    const response = await fetch(fetchUrl, { credentials: "include", redirect: "follow" })
