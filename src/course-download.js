(() => {
  ...
  const TERMINAL_JOB_STATES = new Set(["complete", "error"]);
  ...
  async function startSession(course, scopeTitle, resources) {
    closeRoot();
    panelMinimized = false;
  ...
  async function downloadFileResource(resource) {
    const extension = await resolveResourceExtension(resource);
  ...
  async function discoverVideoCandidates(url) {
    const staticCandidates = await discoverStaticVideoCandidates(url);
    ...
    const loaded = new Promise((resolve, reject) => {
      const timeout = window.setTimeout(() => reject(new Error("Trang video tải quá lâu.")), 25_000);
    ...
    for (let attempt = 0; attempt < 6; attempt += 1) {
      await delay(attempt === 0 ? 2600 : 1400);
  ...
  async function discoverStaticVideoCandidates(url) {
    try {
      ...
      const vimeoEmbeds = Array.from(page.querySelectorAll('iframe[src*="player.vimeo.com/video/"]'))
        .map((frame) => frame.getAttribute("src"))
        .filter(Boolean)
        .map((source) => new URL(source, documentUrl).href);

      for (const embedUrl of new Set(vimeoEmbeds)) {
        const vimeoCandidates = await collectVimeoEmbedCandidates(embedUrl);
        vimeoCandidates.forEach((candidate) => addMediaCandidate(candidates, candidate.url, candidate.source));
      }
      return dedupeMediaCandidates(candidates);
    } catch {
      return [];
  ...
  async function collectVimeoEmbedCandidates(embedUrl) {
    try {
      ...
      const page = new DOMParser().parseFromString(await response.text(), "text/html");
      const config = readEmbeddedVimeoConfig(page);
      if (!config) return [];
      return vimeoCandidatesFromConfig(config);
    } catch {
      return [];
    }
  }
  ...
  function vimeoCandidatesFromConfig(config) {
    const candidates = [];
    const files = config?.request?.files;
    (files?.progressive || []).forEach((file) => {
      if (file?.url) candidates.push({ url: file.url, source: `vimeo-config:${file.quality || "progressive"}` });
    });

    const hls = files?.hls;
    const defaultCdn = hls?.default_cdn;
    if (defaultCdn && hls?.cdns?.[defaultCdn]?.url) {
      candidates.push({ url: hls.cdns[defaultCdn].url, source: "vimeo-config:hls-default" });
    }
    Object.values(hls?.cdns || {}).forEach((cdn) => {
      if (cdn?.url) candidates.push({ url: cdn.url, source: "vimeo-config:hls" });
    });
    return candidates;
  }
  ...
  async function resolveResourceExtension(resource) {
  ...
  function scanResources(scope, course) {
    return Array.from(scope.querySelectorAll(".activity")).flatMap((activity) => {
      ...
      const sourceUrl = link instanceof HTMLAnchorElement ? link.href : "";
      ...
      return [{
        ...
        availability,
        restriction: restriction || (sourceUrl ? "" : "ELOLMS chưa cung cấp link truy cập."),
  ...
  function sectionPath(activity) {
    ...
    while (current instanceof HTMLElement) {
      ...
      if (title) sections.unshift({
        ...
        title,
        directory: sanitizeSegment(title, 32)
      });
  ...
  function assignLocalPaths(resources, course) {
    ...
    resources.forEach((resource) => {
      const relativeDirectory = resource.sections.map((section) => section.directory).filter(Boolean).join("/");
      const downloadDirectory = [COURSE_ROOT_FOLDER, sanitizeSegment(course.title, 32), relativeDirectory].filter(Boolean).join("/");
  ...
  async function downloadManifest(session) {
    ...
    const filename = `${COURSE_ROOT_FOLDER}/${sanitizeSegment(session.course.title, 32)}/ou-yeah-course-manifest.json`;
    const response = await sendRuntimeMessage({
      ...
      <aside class="ou-yeah-course-download-panel" aria-live="polite">
        <div class="ou-yeah-course-download-panel-head">
          <div>
            ...
            <small>${processed}/${readyTotal} mục · ${counts.locked} bị khóa · ${counts.failed} lỗi</small>
          </div>
        ...
        <div class="ou-yeah-course-download-actions">
          ${isActive ? '<button type="button" data-ou-download-pause>Tạm dừng</button><button type="button" data-ou-download-cancel>Hủy</button>' : ""}
  ...
  function requestCancel() {
    ...
    const current = document.querySelector(".ou-yeah-course-download-current");
    if (current) current.textContent = "Sẽ hủy sau tệp hiện tại…";
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #${ROOT_ID} { position:relative; z-index:2147483000; font-family:"Space Grotesk","Segoe UI",sans-serif; }
      ...
      .ou-yeah-course-download-panel { position:fixed; right:22px; bottom:22px; width:min(440px,calc(100vw - 32px)); overflow:hidden; border:1px solid #dfe3ec; border-radius:16px; background:#fff; color:#1d2330; box-shadow:0 18px 54px rgba(25,35,61,.2); }
      .ou-yeah-course-download-panel-head { display:flex; justify-content:space-between; gap:15px; padding:15px 16px 10px; }
      ...
      .ou-yeah-course-download-current { padding:9px 16px; color:#626a78; font-size:11px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      ...
      .ou-yeah-course-download-actions { display:flex; justify-content:flex-end; gap:7px; padding:0 16px 14px; }
      .ou-yeah-course-download-actions button { padding:7px 10px; border:1px solid #dce1ea; border-radius:9px; background:#fff; color:#48505e; font:650 12px/1 "Space Grotesk","Segoe UI",sans-serif; cursor:pointer; }
      ...
      @media (max-width:700px) { #${TOOLBAR_ID} { margin-right:5px; } .${BUTTON_CLASS}.is-course { padding:7px 9px; } .ou-yeah-course-download-summary { grid-template-columns:1fr; } .ou-yeah-course-download-types { flex-wrap:wrap; } .ou-yeah-course-download-minimized { right:16px; bottom:72px; max-width:calc(100vw - 32px); } }
    `;
