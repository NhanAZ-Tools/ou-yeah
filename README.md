...
- Trên trang quiz tự đánh giá, bấm `Bắt đầu quét bộ đề` trong bảng `OU Yeah! Quiz Lab`. Tiện ích xử lý các lượt trong một trang nền cùng domain, còn trang đang nhìn được giữ nguyên để theo dõi hoặc bấm `Tạm dừng`; sau đó có thể `Tiếp tục quét` mà không mất câu đã gom. Bạn có thể đổi sang tab khác nhưng nên giữ tab quiz mở; nếu reload, đóng tab hoặc rời trang khi đang quét/đóng gói, Chrome sẽ hỏi xác nhận. Khi quét xong, bảng kết quả tự thu thành một hàng gọn với `Tải bộ đề` và `Quét bổ sung`; tiện ích chỉ tạo/tải `quiz-bank.zip` sau khi bạn bấm `Tải bộ đề`. Quét bổ sung giữ nguyên ngân hàng hiện tại rồi tìm thêm câu mới, không phải quét lại từ đầu.
