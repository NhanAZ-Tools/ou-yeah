...
test("deadline page renders cached data first and refreshes volatile state in the background", async () => {
  ...
  assert.match(source, /TEMPORARY_MEETING_RETENTION = 7 \* 24 \* 60 \* 60 \* 1000/)
  assert.match(source, /retainMissingMeetingsTemporarily/)
  assert.match(source, /ou-deadline-status-temporary/)
  assert.match(source, /ELOLMS đã ẩn buổi VC khỏi lịch sau khi điểm danh/)
  ...
  assert.match(source, /function mergeDuplicateEvents\(existing, incoming\)/)
  assert.match(source, /event\.date\.getTime\(\)\}\|\$\{normalizeText\(event\.course\)\}\|\$\{resource\}/)
  assert.match(source, /incomingTitle\.length > existingTitle\.length \? incoming : existing/)
  assert.match(source, /DEADLINE_CACHE_VERSION = 4/)
  assert.match(source, /function parseCourseMeetingForumUrls\(html, courseUrl\)/)
  assert.match(source, /a\[href\*="\/mod\/forum\/view\.php"\]/)
  assert.match(source, /function fetchCourseMeetingEvents\(forumUrls(?:, courseUrl = "")?\)/)
  assert.match(source, /function parseCourseMeetingForumEvents\(html, forumUrl(?:, courseUrl = "")?\)/)
  assert.match(source, /data-region="discussion-list-item"/)
  assert.match(source, /function parseMeetingSchedule\(value\)/)
  assert.match(source, /function cleanMeetingTitle\(value\)/)
  assert.match(source, /return isMeetingText\(title\) \? "meeting" : "deadline"/)
  assert.match(source, /DEADLINE_TIME_ZONE = "Asia\/Ho_Chi_Minh"/)
  assert.match(source, /DEADLINE_TIME_ZONE_OFFSET_MINUTES = 7 \* 60/)
  assert.match(source, /function createHanoiDate\(/)
  assert.match(source, /function isSameHanoiDay\(/)
  assert.match(source, /function startDeadlineCountdown\(/)
  assert.match(source, /data-ou-deadline-countdown/)
  assert.match(source, /ou-deadline-row-today-pending/)
  assert.match(source, /formatDeadlineCountdown\(eventTime - now\)/)
  ...
  assert.match(source, /ou-deadline-row-extension-actionable/)
  assert.match(source, /ou-deadline-row-extension-inactive/)
  ...
  assert.match(source, /ou-deadline-row-base/)
  assert.match(source, /ou-deadline-pair \.ou-deadline-row\.ou-deadline-row-base \{ background: linear-gradient/)
  assert.match(source, /ou-deadline-pair \.ou-deadline-row\.ou-deadline-row-base:hover/)
  assert.match(source, /data-ou-deadline-completion-filter/)
  assert.match(source, /hide-completed/)
  assert.match(source, /ou-deadline-completion-menu/)
  assert.match(source, /ou-deadline-completion-option\[aria-selected="true"\]/)
  assert.match(source, /setupCompletionFilter/)
  assert.match(source, /ou-deadline-course-trigger, #\$\{DEADLINE_DASHBOARD_ID\} \.ou-deadline-search input .*height: 40px/)
  assert.match(source, /ou-deadline-completion-trigger .*height: 40px/)
  assert.match(source, /ou-deadline-filters .*align-items: center; flex: 1 1 auto; gap: 8px; height: 40px/)
  assert.match(source, /ou-deadline-course-filter .*height: 40px/)
  assert.match(source, /ou-deadline-completed-filter .*height: 40px/)
  assert.match(source, /ou-deadline-month-trigger .*height: 40px; min-width: 176px; min-height: 40px/)
  assert.match(source, /ou-deadline-month-nav > button .*width: 40px; height: 40px/)
  assert.match(source, /filterCompletedDeadlineEntry/)
  assert.match(source, /isDeadlineCompletedForFilter/)
  assert.match(source, /ou-deadline-footer/)
  assert.match(source, /data-ou-deadline-export/)
  ...
  assert.match(source, /ou-deadline-row-overdue \{ opacity: \.64; \}/)
...
test("ELOLMS course view gets a compact Course Map", async () => {
  ...
  assert.match(source, /data-course-map-action="close" aria-label="Đóng Course Map"/)
  assert.match(source, /function closeCourseMapDrawer\(drawer\)/)
  assert.match(source, /drawer\.querySelector\("\[data-action='closedrawer'\]"\)/)
  assert.match(source, /data-course-map-action="toggle-all" aria-label="Mở rộng tất cả" aria-pressed="false"/)
  assert.match(source, /setAllCourseMapSectionsExpanded\(courseIndex, !areAllCourseMapSectionsExpanded\(courseIndex\)\)/)
  assert.match(source, /function areAllCourseMapSectionsExpanded\(courseIndex\)/)
  assert.match(source, /function updateCourseMapBulkToggle\(courseIndex, tools\)/)
  assert.match(source, /toggle\.textContent = label/)
  assert.match(source, /function ensureCourseMapProgress\(drawer, tools\)/)
  assert.match(source, /drawer\.querySelector\("\.drawerheader \.progress"\)/)
  assert.match(source, /progress\.classList\.add\("ou-course-map-progress"\)/)
  assert.match(source, /function setAllCourseMapSectionsExpanded\(courseIndex, expanded\)/)
  const courseMapBulkToggleSource = source.slice(
    source.indexOf("function setAllCourseMapSectionsExpanded"),
    source.indexOf("function annotateCourseMap")
  )
  assert.match(source, /function getCourseMapSectionToggles\(courseIndex\)/)
  assert.match(source, /courseIndex\.querySelectorAll\(\s*"\[data-for='section'\] > \[data-for='section_item'\] \[data-toggle='collapse'\], " \+/)
  assert.match(courseMapBulkToggleSource, /isCourseSectionToggleOpen\(toggle\) !== expanded/)
  assert.match(courseMapBulkToggleSource, /setCourseMapSectionExpanded\(toggle, expanded\)/)
  assert.match(source, /function setCourseMapSectionExpanded\(toggle, expanded\)/)
  assert.match(source, /target\.classList\.toggle\("show", expanded\)/)
  assert.match(source, /toggle\.classList\.toggle\("collapsed", !expanded\)/)
  assert.doesNotMatch(courseMapBulkToggleSource, /toggle\.click\(\)/)
  assert.doesNotMatch(courseMapBulkToggleSource, /setGeneralCourseSectionExpanded|#collapsesections/)
  assert.match(source, /#\$\{COURSE_MAP_TOOLS_ID\} \.ou-course-map-progress/)
  assert.match(source, /#\$\{COURSE_MAP_TOOLS_ID\} \.ou-course-map-heading-actions/)
  assert.match(source, /#\$\{COURSE_MAP_TOOLS_ID\} \.ou-course-map-actions/)
  assert.match(source, /#\$\{COURSE_MAP_TOOLS_ID\} \{\s*position: relative;\s*flex: 0 0 auto;\s*z-index: 20;/)
  assert.match(source, /#\$\{COURSE_MAP_TOOLS_ID\} \{[\s\S]*background: #fff;[\s\S]*isolation: isolate;/)
  assert.match(source, /\.ou-yeah-course-map-drawer \.drawercontent \{[\s\S]*?padding: 8px 8px 16px !important;/)
  assert.match(source, /\.ou-yeah-course-map-drawer \.drawerheader \{\s*display: none !important;/)
  assert.match(source, /data-target="theme_boost-drawers-courseindex"\] \{\s*display: inline-grid !important;/)
  assert.match(source, /border-radius: 12px !important;/)
  ...
  assert.match(source, /#\$\{COURSE_MAP_TOOLS_ID\} \{\s+position: relative;\s+flex: 0 0 auto;[\s\S]*?gap: 6px;[\s\S]*?padding: 8px 9px;/)
  assert.match(source, /\.ou-yeah-course-map-drawer \.drawercontent \{[\s\S]*?flex-direction: column;[\s\S]*?overflow: hidden !important;/)
  assert.match(source, /#courseindex\.ou-yeah-course-map \{[\s\S]*?flex: 1 1 auto;[\s\S]*?overflow-y: auto;/)
